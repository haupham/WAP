(function( $ ) {
	...
	$.Shop.prototype = {
		init: function() {
			...
			this.cartPrefix = "Perfume-"; // Prefix string to be prepended to the cart's name in the session storage
			...
			this.paypalBusinessEmail = "jbduchau@gmail.com"; // Your Business PayPal's account email address
			...
			if( this.$formCart.length ) {
				...
				if( items.length == 0 ) {
					...
					for( var i = 0; i < items.length; ++i ) {
						...
						var qty = item.qty;
						var html = "<tr><td class='pname'>" + product + "</td>" + "<td class='pqty'><input type='number' min='0' max='9999' value='" + qty + "' class='qty'/></td>";
